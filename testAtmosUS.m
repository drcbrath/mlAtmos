...
% 1. Validate AtmosUS, on std day by comparison to 
...
% 4. Validate AtmosUS by comparison to verified AtmosUS results multiplied by 
%    conversion factors.
	% 4.1 std day
	% 4.2 dT offset day
	% 4.3 custom profile
...
% same as in testAtmosUS, but converted to feet
H = [  -100; 0.0; 5500; 11000.0; 15500; 20000.0; 26000; 32000.0; 39500; 47000.0; 49000; 51000.0; 61000; 71000.0; 80000; 84852.0; 90000 ]/0.3048;
dT = 30*1.8;   % same as in testAtmosSI but convert from degK to degR


% (1) standard day, for comparison to US Standard Atmosphere, 1976, reference work
[T_std, rho_std, P_std, a_std, visc_std, theta_std, sigma_std, delta_std, kappa_std] = AtmosUS(H);    % compare to old version
...
H_L = 11000/0.3048;       % test point, top of troposphere (converted to feet)
[T_2L_dT, rho_2L_dT, P_2L_dT] = AtmosUS(H_L,dT);   % with dT = 20, should match hand calc in linear layer

% isothermal layer
H_I = 15500/0.3048;   % test point (converted to feet)
[T_2I_dT, rho_2I_dT, P_2I_dT] = AtmosUS(H_I,dT);   % with dT = 20, should match hand calc in linear layer
...
[T_dT, rho_dT, P_dT, a_dT, visc_dT, theta_dT, sigma_dT, delta_dT, kappa_dT] = AtmosUS(H,dT);    % compare to old version
...
Hk     = [   0.0; 11000.0; 20000.0; 32000.0; 47000.0; 51000.0; 71000.0; 84852.0 ]/0.3048;   % ft
...
[T_cus, rho_cus, P_cus, a_cus, visc_cus, theta_cus, sigma_cus, delta_cus, kappa_cus] = AtmosUS(H,H_custom,T_custom);    % compare to old version


%% Results
fid = fopen('testAtmosUS.dat','wt');
...
fprintf(fid,'    (ft)        (R)  (sl/ft^3) (lbf/ft^2)     (ft/s)        (1)        (1)        (1)        (1) (lbf*s/ft^2)\n');
fprintf(fid,'%8.1f %10.4e %10.4e %10.4e %10.4e %10.4e %10.4e %10.4e %10.4e   %10.4e\n',[H, T_std, rho_std, P_std, a_std, theta_std, sigma_std, delta_std, kappa_std, visc_std]');
...
fprintf(fid,'       H          T          P\n');
fprintf(fid,'    (ft)        (R) (lbf/ft^2)\n');
fprintf(fid,'%8.1f %10.4e %10.4e --- linear layer with dT=%4.0f\n',H_L,T_2L_dT, P_2L_dT, dT);
fprintf(fid,'%8.1f %10.4e %10.4e --- isothermal layer with dT=%4.0f\n',H_I,T_2I_dT, P_2I_dT, dT);
